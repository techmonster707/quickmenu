...
if (!function_exists('glanguage')) {
    ...
    if(!function_exists('genKey')){
		function genKey($code, $site_url, $a_code){
			//$settings = settings();
			return sha1($code.$site_url.$a_code);
		}
	}
		...
		function siteID($ste, $a ='')
		{
			$enc_met = "AES-256-CBC";
			$s_key = 'BHWEMS4HT61GULKSET9X4Y';
			$s_iv = 'j5yOs85pzer';
			$key = hash('sha256', $s_key);
			$iv = substr(hash('sha256', $s_iv), 0, 16); 
			switch($a){
				case 'e':
					$output = openssl_encrypt($ste, $enc_met, $key, 0, $iv);
					$output = base64_encode($output);
					break;
				case 'd':
					$output = openssl_decrypt(base64_decode($ste), $enc_met, $key, 0, $iv);
					break;	
				default:
					$resp  = json_encode(['success'=>false]);
					$output = $resp;
					break;
			}
			return $output;		
